...
class HttpRequestMetrics {
    ...
    onResponse(response) {
        ...
        response.on("error", (e) => {
            debug("⚡️ [response] error");
            console.log('⚡️ [response] error', e);
        });

        response.on("pipe", (src) => {
            debug("⚡️ [response] pipe");
            console.log('⚡️ [response] pipe', src);
        });
        ...
        this.startNoResponseEndTimer(response);
    }

    startNoResponseEndTimer(response) {
