...
const MS_PER_NS = 1e6;
const METRICS_SENDING_BUFFER_MS = 10000;
...
class HttpRequestMetrics {
    ...
    startNoResponseEndTimer() {
        ...
        let waitTime = 5*60*1000; // 5 minute fallback in case we don't have the __OW_DEADLINE (unexpectedly)
        if (process.env.__OW_DEADLINE) {
            // add 10 second buffer to be able to send metrics
            waitTime = process.env.__OW_DEADLINE - Date.now() - METRICS_SENDING_BUFFER_MS;
        }
