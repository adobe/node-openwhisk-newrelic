...
class HttpRequestMetrics {
    ...
    onResponse(response) {
        ...
        if (isNaN(this.responseSize)) {
            ...
            // prevent stream from being read as soon as we add our 'data' listener below,
            // leave that to the actual client code consuming the request
            response.readableFlowing = false;
            ...
            // reset flowing state so that as soon as client code adds a data listener
            // it will start consuming the stream
            response.readableFlowing = null;
        }

        response.on("end", () => {
            debug("⚡️ [response] end");
            console.log('⚡️ [response] end');
            // all of response body was received
            this.onEnd();
        });

        response.on("close", () => {
            debug("⚡️ [response] close");
            console.log('⚡️ [response] close');
            // all of response body was received
            this.onEnd();
        });

        this.startResponseTimers(response);
    }

    startResponseTimers(response) {
        ...
        let waitTime;
        // ensure stream is flowing at 10 seconds - addresses bug were response stream is
        // never consumed and end event does not fire
        let ensureStreamWaitTime = 10000;
        if (process.env.__OW_DEADLINE) {
            ...
            // check to ensure stream is flowing at either 10 seconds or calculated waitTime if less
            if (waitTime < 10000) ensureStreamWaitTime = waitTime;
        } else {
            // 5 minute fallback in case we don't have the __OW_DEADLINE (unexpectedly)
            waitTime = 5*60*1000;
        }

        // set listener to confirm stream is flowing (i.e. client code is reading stream), otherwise
        // manually start the stream to ensure end event fires
        this.responseStreamTimer = setTimeout(() => {
            if (!response.readableFlowing) {
                console.log(`http-client metrics: response stream paused after ${ensureStreamWaitTime} ms; manually resuming stream`);
                response.resume();
            }
        }, ensureStreamWaitTime);

        this.noResponseEndTimer = setTimeout(() => {
            console.log(`http-client metrics: did not receive response 'end' event after ${waitTime} ms; response.readableFlowing=${response.readableFlowing}`);
            this.onEnd();
        }, waitTime);
    }
    ...
    triggerMetrics(extraMetrics) {
        ...
        if (this.responseStreamTimer) {
            clearTimeout(this.responseStreamTimer);
        }
    ...
    getRequestMetrics() {
        ...
        let protocol = opts.protocol || req.protocol;
        if (!protocol) {
            if (opts.agent && opts.agent.protocol) {
                protocol = opts.agent.protocol;
            } else {
                // "request" library does not set protocol in case of https, must deduct from opts.href
                protocol = (opts.href && opts.href.startsWith("https:")) ? "https:" : "http:";
            }
        }
