...
class NewRelic {
     ...
     * @param {boolean} autoStart (optional), set to true by default to immediately start sending metrics after construction
     */
    constructor(options) {
    ...
    start(finishingCb=null){
        if(!this.currentlySampling && !this.disableActionTimeout) {
    ...
    close() {
        ...
        this.actionTimeoutHandlerId = null;
        this.currentlySampling = false;
    }
...
function sendMetricsOnActionTimeout(self, actionTimeoutMetricsCb, finishingCallback) {
    return setTimeout(
        async () => {
            ...
            if(finishingCallback && typeof finishingCallback === 'function'){
                finishingCallback();
            }
        },
