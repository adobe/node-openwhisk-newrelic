...
class NewRelic {
    ...
    async send(eventType, metrics={}) {
        console.log('Sending metrics (self.send)');
        try {
            ...
            console.log('*****Sending metrics (self.send) =====> done');
        } catch (e) {
            console.log(e);
...
/**
    ...
    function sendMetricsOnActionTimeout(self, actionTimeoutMetricsCb, finishingCallback) {
        return setTimeout(
            async () => {
                console.log('Inside function that should have timed out');
                let metrics = {};
