...
class NewRelic {
    ...
    constructor(options) {
        if (!options || !options.newRelicEventsURL || !options.newRelicApiKey) {
            ...
            this.sendMetricsOnActionTimeout(options.setActionTimeoutMetricsCB);
     ...
     */
    sendMetricsOnActionTimeout(actionTimeoutMetricsCB) {
        const timeUntilTimeout = Metrics.timeUntilTimeout();
        this.actionTimeoutInterval = setTimeout(
            async (metricsCB) => {
                let metrics = {};
                ...
                await this.send("timeout", metrics);
                console.log(`Metrics sent before action timeout.`);
            },
            timeUntilTimeout - TIMEOUT_BUFFER, actionTimeoutMetricsCB );
    }

    clearActionTimeoutInterval() {
        clearTimeout(this.actionTimeoutInterval);
    }
    ...
    async send(eventType, metrics) {
        try {
            ...
            if (!response.ok) {
                ...
                console.log(`Metrics sent to NewRelic: ${JSON.stringify(response.json())}`)
            }
        } catch (e) {
