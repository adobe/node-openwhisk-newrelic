...
/**
 * Verify if a string is blank. Any other type than string will be considered blank.
 *
 * @param {String} str string value to check
 * @returns {Boolean} true if string is blank, false otherwise
 */
...
class NewRelic {
    ...
    constructor(options, defaultMetrics={}) {
        if (!options || 
            ...
            if(!validUrl.isWebUri(options.newRelicEventsURL)) {
                this.canSendMetrics = false;
                console.error('NewRelic URL is not a valid URL. Metrics disabled.');
            } else {
    ...
    async send(eventType, metrics={}) {
        if (!this.canSendMetrics) return Promise.resolve(); 
