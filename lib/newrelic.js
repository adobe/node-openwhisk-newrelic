...
async function sendQueuedMetrics(self) {
    ...
    try {
        console.log(`sending ${self.queuedMetrics.length} queued metrics`);
        ...
        // reset timer so that the next NewRelic.send() would start a new timer
        if (self.sendTimeoutId) {
            clearTimeout(self.sendTimeoutId);
            delete self.sendTimeoutId;
        }
    }
}
