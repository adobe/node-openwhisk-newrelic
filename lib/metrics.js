...
/**
 * Container memory size information from the environment
 *
 * @param {Object} [metrics={}] Metrics object
 * @returns {Object} Metrics object
 */
function containerMemory(metrics) {
    metrics = metrics || {};
    try {
        metrics.containerMemorySize = parseFloat(fs.readFileSync('/sys/fs/cgroup/memory/memory.limit_in_bytes').toString()) || undefined;

    // eslint-disable-next-line no-unused-vars
    } catch (e) {
        // ignore error if not running in a docker container
    }
    return metrics;
}
