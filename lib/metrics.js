...
const CONTAINER_MEMORY_SIZE_V1 = '/sys/fs/cgroup/memory/memory.limit_in_bytes';
const CONTAINER_MEMORY_SIZE_V2 = '/sys/fs/cgroup/memory.max';
...
function openwhisk(metrics) {
    ...
    try {
        ...
        if (fs.existsSync(CONTAINER_MEMORY_SIZE_V2)) {
            // cgroup v2
            metrics.containerMemorySize = parseInt(fs.readFileSync(CONTAINER_MEMORY_SIZE_V2), 10) || undefined;
        } else {
            // cgroup v1
            metrics.containerMemorySize = parseInt(fs.readFileSync(CONTAINER_MEMORY_SIZE_V1), 10) || undefined;
        }
