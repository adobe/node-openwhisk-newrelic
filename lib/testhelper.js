...
// load optional dependency nock only on demand as it is only used in tests
// and we don't want nock as a normal production dependency
function nock() {
    // this is to fool webpack to not include this (we only need this for tests)
    // (leads to warning "Critical dependency: the request of a dependency is an expression")
    const NOCK = "nock";
    return require(NOCK);
}
        ...
        assert(
            actualArray.some((e) => {
                try {
                    ...
                    return true;
                }  /* eslint-disable no-unused-vars */ catch (err) {
                    return false;
                }
            }),
